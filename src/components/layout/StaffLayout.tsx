...
export function StaffLayout() {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col md:flex-row theme-staff">
      {/* Mobile Top Bar */}
      <div className="md:hidden glass border-b border-white/5 p-4 flex items-center justify-between sticky top-0 z-50">
        <Link to="/" className="flex items-center gap-2">
          <div className="w-8 h-8 rounded-lg bg-primary flex items-center justify-center font-bold text-white text-sm">FS</div>
        ...
        {(isMobileOpen || window.innerWidth >= 768) && (
          ...
          >
            <div className="hidden md:flex items-center gap-3 mb-12 mt-2">
              <div className="w-10 h-10 rounded-xl bg-primary flex items-center justify-center font-bold text-white text-lg shadow-[0_0_15px_rgba(16,185,129,0.4)]">FS</div>
                  ...
                  <Link key={item.path} to={item.path} onClick={() => setIsMobileOpen(false)}>
                    <div
                      className={cn(
                        ...
                        isActive
                          ? "bg-primary/10 text-primary border border-primary/20 shadow-[0_4px_15px_rgba(16,185,129,0.05)]"
