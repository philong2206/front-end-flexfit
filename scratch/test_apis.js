...
async function runTests() {
  ...
  try {
    ...
    adminId = adminLoginRes.user.userId;
    console.log(`=> Đăng nhập Admin thành công. Token: ${adminToken.substring(0, 15)}...`);

    // 3. Đăng nhập Member
    console.log("[3] Đăng nhập tài khoản Member...");
    const memberLoginRes = await login(MEMBER_EMAIL, '123456');
    memberToken = memberLoginRes.token;
    memberId = memberLoginRes.user.userId;
    console.log(`=> Đăng nhập Member thành công. Token: ${memberToken.substring(0, 15)}...`);
  ...
  // ==========================================
  // Hạng mục 1: Test API Quản lý Gói Tín dụng (CreditPackage)
  // ==========================================
